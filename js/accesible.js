...
// ── Inicializar pestaña Accesible (Render Dinámico y Resiliente) ──
window.RenderHormigasAccesible = function() {
  const sec = document.getElementById('accesible');
  if (!sec) return;
  if (sec.innerHTML.trim() !== "") return; // Evita duplicar la interfaz
    ...
    <div style="max-width:680px;margin:0 auto;">
      ...
      <div style="background:#1a1a1a;border:1px solid #2a2a2a;border-radius:12px;padding:1.5rem;margin-bottom:1.5rem;">
        ...
        <p style="font-size:0.75rem;color:#666;margin-bottom:1rem;line-height:1.7;">
          Ingresa una firma CLHQ-XXXXXXXX para ver su representation en Braille Unicode y escucharla en voz alta.
        </p>
...
async function verificarConVoz() {
  ...
  try {
    const r = await fetch(window.API ? window.API : '/api' + '/seal/' + firma);
...
// Inicialización persistente en el entorno del Borde
document.addEventListener('DOMContentLoaded', function() { window.RenderHormigasAccesible(); });
setInterval(function() { window.RenderHormigasAccesible(); }, 1000);
